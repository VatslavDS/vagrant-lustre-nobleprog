...
#        vb.customize ["storagectl", :id, "--name", "SATA Controller", "--add", "sata" ]
      ...
      if !File.exist?("mgt01.vdi")
        vb.customize ["createhd", "--filename", "mgt01.vdi", "--size", 8192, "--variant", "Fixed"]
      ...
      if !File.exist?("mgt02.vdi")
        vb.customize ["createhd", "--filename", "mgt02.vdi", "--size", 8192, "--variant", "Fixed"]
        vb.customize ["modifyhd", "mgt02.vdi", "--type", "shareable"]
      end
      vb.customize ["storageattach", :id, "--storagectl", "SATA Controller", "--port", 5, "--device", 0, "--type", "hdd", "--medium", "mgt02.vdi"]


      # 1 GB Volume for Lustre MDT (Metadata)
      if !File.exist?("mdt01.vdi")
        vb.customize ["createhd", "--filename", "mdt01.vdi", "--size", 8192, "--variant", "Fixed"]
      ...
      if !File.exist?("mdt02.vdi")
        vb.customize ["createhd", "--filename", "mdt02.vdi", "--size", 8192, "--variant", "Fixed"]
        vb.customize ["modifyhd", "mdt02.vdi", "--type", "shareable"]
      end
      vb.customize ["storageattach", :id, "--storagectl", "SATA Controller", "--port", 6, "--device", 0, "--type", "hdd", "--medium", "mdt02.vdi"]
      ...
      vb.customize ["storageattach", :id, "--storagectl", "SATA Controller", "--port", 7, "--device", 0, "--type", "hdd", "--medium", "ost02.vdi"]
    end
  ...
  config.vm.define "compute01" do |compute01|
   ...
   #ompute01.vm.provision "shell", path: "compute-2-nfs_client.sh"
   #compute01.vm.provision "shell", path: "compute-3-slurm_client.sh"
  end

  # Compute node 2
  config.vm.define "compute02" do |compute02|
    compute02.vm.box = "manager-for-lustre/lustre-client-el7"
